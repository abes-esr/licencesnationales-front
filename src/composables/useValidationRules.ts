...
export const useValidationRules = () => {
  ...
  return {
    establishmentNameRules: [
      (v: string) => !!v || t("validation.establishmentNameRules.required")
    ],
    sirenRules: [
    ...
    establishmentTypeRules: [
      (v: string) => !!v || t("validation.establishmentTypeRules.required")
    ],
    ...
    privacyAcceptanceRules: [
      (v: boolean) => v || t("validation.privacyAcceptanceRules.required")
    ],
    searchDomainRules: [
      (v: string) => !!v || t("validation.searchDomainRules.required")
    ],
    searchCriteriaRules: [
      (v: string) => !!v || t("validation.searchCriteriaRules.required")
    ],
    ...
    ipv6SegmentRules: [
      (v: string) => !!v || t("validation.ipSegmentRules.required"),
      (v: string) =>
        /^\\s*([0-9a-fA-F]{1,4})$/.test(v) || t("validation.ipSegmentRules.invalid")
    ],
    ipv4Rules: [
    ...
    commentRules: [
      (v: string) => v.length <= 255 || t("validation.commentRules.max255")
    ],
    adminCommentRules: [
